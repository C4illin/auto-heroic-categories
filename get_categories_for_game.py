...
for app_name in game_library.library_dict:
    url = (
        ...
        + app_name
        + "&fields=id,name,genres,multiplayer_modes"
    )
    response = post(
        url,
        **{
            ...
            "data": "fields category,checksum,content_descriptions,rating,rating_cover_url,synopsis,multiplayer_modes;",
        },
    )
    ...
    try:
        ...
        for i in response.json()[0]["multiplayer_modes"]:
            for tag in get_categories.multiplayer_dict(i):
                if (
                    game_library.library_dict[app_name]
                    not in data["games"]["customCategories"][tag]
                ):
                    data["games"]["customCategories"][tag].append(
                        game_library.library_dict[app_name]
                    )
